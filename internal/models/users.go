...
type UserModelInterface interface {
	...
	PasswordUpdate(id uint, currentPassword, newPassword string) error
}
...
func (m *UserModel) Authenticate(email, password string) (uint, error) {
	...
	stmts := "SELECT id, hashed_password FROM users WHERE email = ?"

	err := m.DB.QueryRow(stmts, email).Scan(&id, &hashedPassword)
	if err != nil {
...
func (m *UserModel) PasswordUpdate(id uint, currentPassword, newPassword string) error {
